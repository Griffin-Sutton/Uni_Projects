...
int main() 
{
	...
	// read from the file into the vector 
	if(readFromFile(words)) {
		...
		/*
			Create a traditional for loop to go through each element of 
			vector and modify it. Use an OpenMP directive to parallelize 
			the for loop.
		*/
		...
		// the printVector function call can be commented out after you test 
		// cout << "\n\nAfter sorting:\n";
		// printVector(words);
		...
		cout << string(40, '*') << endl;
		cout << "Computation time for OpenMP parallel = " << compute_time_s << " seconds." << endl << endl;
	}
