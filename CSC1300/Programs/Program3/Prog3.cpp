...
int main() 
{
    ...
    try 
	{
       ...
       json features = jsonObject["features"];
		...
		cout << "Earthquakes that caused a tsunami: " << endl;
        int tsunamiCount = 1;
        
		// Iterating through each element in the "features" array again
		for (unsigned int i = 0; i < features.size(); ++i) {
			// Accessing each earthquake's data by indexing
			json element = features[i];

			// We check if this particular earthquake caused a tsunami.
			if (element["properties"]["tsunami"] == 1) {
				cout << "Earthquake " << tsunamiCount 
					<< ": \n\tPlace: " << element["properties"]["place"] 
					<< "\n\tMagnitude: " << element["properties"]["mag"] << endl;
				cout << endl;
				tsunamiCount++;
